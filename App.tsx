...
import { Lead, CRMStatus, LeadCategory, BuyingIntent, Urgency, BuyingStage, MessageSource, AppSettings } from './types';
...
const INITIAL_LEADS: Lead[] = [
  {
    ...
    platform: MessageSource.INSTAGRAM_DM,
    message_text: "Hi! I love the new collection. How much is the velvet sofa and do you ship to NY?",
    timestamp: new Date().toISOString(),
    crm_status: CRMStatus.NEW,
    tags: ['Furniture', 'US-East'],
    history: [{ id: 'h1', date: new Date().toISOString(), type: 'system', content: 'Lead captured from Instagram' }],
    analysis: {
      category: LeadCategory.HOT,
      intent: BuyingIntent.EXPLICIT,
      urgency: Urgency.HIGH,
      stage: BuyingStage.DECISION,
      product_detected: "Velvet Sofa",
      recommended_action: "Send Pricing",
      score: 92,
      summary_notes: "Explicit price query with delivery location."
    }
  },
  {
    id: '2',
    username: 'mike_tech_ceo',
    platform: MessageSource.LINKEDIN_MSG,
    message_text: "We are looking to upgrade our office CRM. Does your tool support API integrations?",
    timestamp: new Date(Date.now() - 86400000).toISOString(),
    crm_status: CRMStatus.CONTACTED,
    tags: ['B2B', 'Enterprise'],
    history: [
      { id: 'h2', date: new Date(Date.now() - 86400000).toISOString(), type: 'system', content: 'Lead captured from LinkedIn' },
      { id: 'h3', date: new Date(Date.now() - 43200000).toISOString(), type: 'status_change', content: 'Status changed to Contacted' }
    ],
    analysis: {
      category: LeadCategory.HOT,
      intent: BuyingIntent.EXPLICIT,
      ...
      product_detected: "CRM Tool",
      recommended_action: "Schedule Demo",
      score: 88,
      summary_notes: "B2B inquiry regarding API capabilities."
    }
  }
];
...
export default function App() {
  ...
  const handleUpdateStatus = (leadId: string, newStatus: CRMStatus) => {
    setLeads(leads.map(lead => {
      if (lead.id === leadId) {
        ...
        return { ...lead, crm_status: newStatus, history: updatedHistory };
      }
      return lead;
    }));
  ...
  const handleAddNote = () => {
    ...
    setLeads(leads.map(lead => {
      if (lead.id === selectedLead.id) {
        return { ...lead, history: [noteEntry, ...lead.history] };
      }
      return lead;
    }));

    // Update locally selected lead to reflect changes immediately
    setSelectedLead(prev => prev ? { ...prev, history: [noteEntry, ...prev.history] } : null);
    setNewNote('');
  };
